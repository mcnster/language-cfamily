...
-----------------------------------------------------------------------------
-- |
-- Module      :  Language.CFamily.C.Parser.Tokens
...
--
-----------------------------------------------------------------------------
module Language.CFamily.C.Parser.Tokens (CToken(..), posLenOfTok, GnuCTok(..)) where

import Language.CFamily.Data.Position    (Position, Pos(..), PosLength)
import Language.CFamily.Data.Ident       (Ident, identToString)
import Language.CFamily.C.Syntax.Constants (CChar, CInteger, CFloat, CString)
...
instance Show CToken where
  ...
  showsPrec _ (CTokGnuC GnuCInt128 _) = showString "__int128"
